...
class Solution:
    def trap(self, heights: List[int]) -> int:
        ...
        #print("Aggregate Rain: ")

        total_rain = 0

        for i in range(0, len(heights)):

            #print("{0}: {1} within peaks [{2}, {3}]".format(i, heights[i], tallest_left_peak[i], tallest_right_peak[i]))

            height = heights[i]
            minimum_contained_height = min(tallest_left_peak[i], tallest_right_peak[i])

            rain_collected = max(0, minimum_contained_height - height)
            total_rain += rain_collected

            #print("Rain Collected: ", rain_collected)
